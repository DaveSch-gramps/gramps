...
class Place(CitationBase, NoteBase, MediaBase, UrlBase, AttributeBase,
    ...
    def __init__(self, source=None):
        ...
        if source:
            ...
            self.type_list = list(map(PlaceTypeRef, source.type_list))
            ...
            self.placeref_list = []
            self.type_list = []  # empty Placetype
            self.alt_loc = []
            self.event_ref_list = []
    ...
    def unserialize(self, data):
        ...
        self.type_list = [PlaceTypeRef().unserialize(typ) for typ in types]
    ...
    def get_handle_referents(self):
        ...
        :rtype: list
        """
        return (self.name_list + self.type_list + self.attribute_list +
                self.placeref_list + self.media_list + self.event_ref_list)
    ...
    def add_name(self, name, sort=True):
        ...
        :type name: string
        :param sort: wether to sort on add
        :type sort: bool
        """
        ...
        if name not in self.name_list:
            if self.name_list and sort:
                if(name.lang == glocale.lang or not name.lang):
                    if(self.name_list[0].lang == glocale.lang or
                       not self.name_list[0].lang):
                        # langs match, now compare dates
                        if(name.date.is_empty() and not
                           self.name_list[0].date.is_empty() or
                           not self.name_list[0].date.is_empty() and
                           self.name_list[0].date < name.date):
    ...
    def set_type(self, place_type_handle):
        """
        ...
        :type type: PlaceType handle (str)
        """
        if not self.type_list:
            self.type_list.append(PlaceTypeRef())
        self.type_list[0].ref = place_type_handle
    ...
    def get_type(self):
        ...
        :rtype: PlaceType handle (str)
        """
        return self.type_list[0].ref
    ...
    def add_type(self, p_type, sort=True):
        ...
        the list.
        If we allow sorting then
                most recent type date is put at the top.
        otherwise we just append
        ...
        :type sort: bool
        """
        if len(self.type_list) == 1 and self.type_list[0].is_empty():
            self.type_list[0] = p_type
            return
        if p_type not in self.type_list:
            if self.type_list and sort:
                if(p_type.date.is_empty() and not
                   self.type_list[0].date.is_empty() or
                   not self.type_list[0].date.is_empty() and
                   self.type_list[0].date < p_type.date):
    ...
    def add_placeref(self, placeref):
        ...
        :param placeref: place reference to append to the list
