...
from gramps.gen.lib import (
    ...
    MediaRef, Name, NameType, Note, NoteType, Person, PersonRef,
    Place, PlaceAbbrev, PlaceAbbrevType,  PlaceName, PlaceHierType,
    PlaceRef, PlaceType,
...
class PlaceParser:
    ...
    def __init__(self, line=None, pdict=None):
        self.pf_list = []
    ...
    def parse_form(self, line):
        """
        Parses the GEDCOM PLAC.FORM into a list of PlaceTypes
        (if possible).
        ...
        for item in line.data.split(','):
            ...
            if type_num == self.POSTAL:
                ...
                ptype = self.pdict.get(item)
                type_num = PlaceType((ptype, item) if ptype else item)
...
class GedcomParser(UpdateCallback):
    ...
    def __init__(self, dbase, ifile, filename, user, stage_one,
        ...
        self.place_loc = stage_one.place_loc
        self.loc_gov = stage_one.loc_gov

        self.place_parser = PlaceParser(pdict=self.place_type_dict)
...
# Despite the name, the following are Gramps ID to handle
        ...
        for place_handle in self.place_names[title]:
            ...
            if place.get_title() == title:
                for typ in place.get_types():
                    if typ.is_equivalent(ptype) != DIFFERENT:
    ...
    def __add_place(self, event, sub_state):
        ...
        if sub_state.place:
            # we have a place
            ptypes = [] if (self.place_loc and sub_state.place.gramps_id)\
                else sub_state.place_pf.get_pf()
            ...
            if len(places) == len(ptypes):
                ...
                sub_state.place.set_name(
                    PlaceName(value=sub_state.place.title))

                place = self.__find_place(sub_state.place.title,
                                          sub_state.place.get_type(), None,
                                          gov=_gov,
                                          xref=_loc)
                if place is None:
                    place = sub_state.place
    ...
    def __place_merge(self, place, acquisition):
        ...
        for addendum in acquisition.name_list:
            for name in name_list:
                if name.value == addendum.value:
                    if name.date.is_empty() and not name.lang:
                        name.date = addendum.date
                        name.lang = addendum.lang
                        name.merge(addendum)
                    elif addendum.date.is_empty() and not addendum.lang:
                        name.merge(addendum)
                    else:
        ...
        for addendum in acquisition.type_list:
            for ptype in type_list:
                if int(ptype) == int(addendum):
                    # look alike, is one definitive?
                    if ptype.date.is_empty():
                        ptype.date = addendum.date
                        ptype.merge(addendum)
                    elif addendum.date.is_empty():
                        ptype.merge(addendum)
                    else:
                        place.add_type(addendum)
                    break
            else:
                if int(addendum) != PlaceType.UNKNOWN:
                    if(place.type_list and
                       place.type_list[0] == PlaceType.UNKNOWN):
    ...
    def __check_xref(self):
        ...
        def __check(_map, objtype, class_func, gramps_id2handle, msg):
            for input_id, gramps_id in _map.map().items():
                ...
                if not self.dbase.method("has_%s_gramps_id",
                                         objtype)(gramps_id):
                    _handle = gramps_id2handle.get(gramps_id)
                    if _handle and self.dbase.method("has_%s_handle",
                                                     objtype)(_handle):
                        continue
                    if msg == "FAM":
        ...
        __check(self.pid_map, 'person', self.__find_or_create_person,
                self.gid2id, "INDI")
        __check(self.fid_map, 'family', self.__find_or_create_family,
                self.fid2id, "FAM")
        __check(self.sid_map, 'source', self.__find_or_create_source,
                self.sid2id, "SOUR")
        __check(self.oid_map, 'media', self.__find_or_create_media,
                self.oid2id, "OBJE")
        __check(self.rid_map, 'repository', self.__find_or_create_repository,
                self.rid2id, "REPO")
        __check(self.nid_map, 'note', self.__find_or_create_note,
                self.nid2id, "NOTE")
        __check(self.lid_map, 'place', self.__find_or_create_place,
                self.lid2id, "PLACE")
...
#            name = self.__parse_name_personal(line.data)
    ...
    def build_lds_ord(self, state, lds_type):
        ...
        sub_state.place_pf = PlaceParser(pdict=self.place_type_dict)
        sub_state.person = state.person
    ...
    def __lds_form(self, line, state):
        ...
        @type state: CurrentState
        """
        state.place_pf = PlaceParser(line, pdict=self.place_type_dict)
    ...
    def __place_name(self, line, state):
        ...
        if sub_state.title:
            self.__do_note(state.place, _('Place Attribute'),
    ...
    def __place_type(self, line, state):
        ...
        """
        type_str = line.data.strip().capitalize()
        ptype = self.place_type_dict.get(type_str)
        ptype = PlaceType((ptype, type_str) if ptype else type_str)
        sub_state = CurrentState()
        sub_state.level = state.level + 1
        ...
        ptype.set_date_object(sub_state.event.date)
    ...
    def __date_cit_type(self, state, one_line=False):
        ...
        if not sub_state.event.date.is_empty():
            text += fmt % (_("Date:"),
                           complete_gedcom_date(sub_state.event.date))
        ...
        while self.locs_list:
            ...
            for (handle, lid) in self.locs_list:
                ...
                if ready:
                    ...
                    fnd_place = self.__find_place(place.title,
                                                  place.get_type(),
                                                  ref_hndl,
                                                  no_find=place.handle)
                    if fnd_place:
            ...
            if len(self.locs_list) != len(locs_list):
                ...
                for (handle, lid) in self.locs_list:
                    visited = []
                    self.dbase.commit_place(__get_title(handle, visited),
                                            self.trans)
                if del_list:
                    self.__add_msg(msg)
                break
    ...
    def __event_addr(self, line, state):
        ...
        if not title:  # indicates that no structured address provided
            ...
            state.addr_pf = [PlaceType(_("Address"))]
            state.addr = [title]
            sub_state.place.set_name(PlaceName(value=title))
        else:
            # structured address provided
            for item, ptype in (
                (addr.get_street(), self.place_type_dict.get("Street")
                 if self.place_type_dict.get("Street") else PlaceType.STREET),
                (addr.get_city(), self.place_type_dict.get("City")
                 if self.place_type_dict.get("City") else PlaceType.CITY),
                (addr.get_state(), self.place_type_dict.get("State")
                 if self.place_type_dict.get("State") else PlaceType.STATE),
                (addr.get_country(), self.place_type_dict.get("Country")
                 if self.place_type_dict.get("Country") else
                 PlaceType.COUNTRY)):
                item = item.replace("\n", " ").strip(',' + string.whitespace)
                if not item:
                    continue    # Don't store empties
                state.addr_pf += [PlaceType(ptype)]
                state.addr += [item]
...
class GedcomStageOne:
    ...
    def __init__(self, ifile, dbase):
        ...
        self.place_loc = False    # file uses _LOC/_LOC style of place tree
        ...
        for line in reader:
            ...
            if key in ("HUSB", "HUSBAND", "WIFE") and \
                ...
                if '1252' in self.enc:
                    ...
                    _type = byte_value.decode('cp1252',
                                              errors='surrogateescape')
                    ...
                    _type = byte_value.decode('latin1',
                                              errors='surrogateescape')
                try:
                    # GOV types are negative in our PlaceTypes
