...
class PlaceBaseModel:
    ...
    def column_type(self, data):
        ptype_hndl = data[7][0][0]
        cached, value = self.get_cached_value(ptype_hndl, "PTYPE_NAME")
        if not cached:
            value = str(self.db.get_placetype_from_handle(ptype_hndl))
            self.set_cached_value(ptype_hndl, "PTYPE_NAME", value)
        return value
