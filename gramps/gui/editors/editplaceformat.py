...
from gramps.gen.lib import (
    PlaceType, PlaceTypeRef, PlaceHierType, PlaceAbbrevType, PlaceGroupType)
...
class EditPlaceFormat(ManagedWindow):
    ...
    def __add_rule(self, _button):
        """ Add a new rule """
        rule = PlaceRule(None, PlaceRule.T_GRP,
    ...
    def __what_changed(self, _obj, use_val=False):
        ...
        if r_type == PlaceRule.T_ST_NUM:
            ...
            self.type_combo = StandardCustomSelector(
                PlaceGroupType().get_map(), type_cb,
                custom_key=PlaceGroupType.CUSTOM,
                additional=self.dbstate.db.get_placegroup_types())
            value = self.rule.value if use_val else PlaceGroupType(
                PlaceGroupType.PLACE)
            ...
            self.ptype.ref = self.rule.value if use_val else PlaceType.UNKNOWN
            self.type_combo = PlaceTypeSelector(self.dbstate.db,
                                                type_cb, self.ptype)
    ...
    def __done(self, _obj, response):
        ...
        if response == Gtk.ResponseType.OK:
            ...
            if self.rule.type == PlaceRule.T_ST_NUM:
                ...
                self.rule.value = self.ptype.ref
                self.rule.detail = self.dbstate.db.get_placetype_from_handle(
                    self.rule.value)
